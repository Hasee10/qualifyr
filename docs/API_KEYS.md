...
| `GTM_MAILBOX_2_USER` / `GTM_MAILBOX_2_PASSWORD` | Gmail | Mailbox #2 for rotation | free | optional | ✅ set |
...
Overture Maps (S3 parquet), OpenStreetMap Overpass + mirrors, Nominatim, GDELT (planned, Phase D),
RDAP/WHOIS (planned, Phase D), SECP / chamber directories (scraped, Phase D), Google Fonts (UI).
