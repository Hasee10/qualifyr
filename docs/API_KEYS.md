...
| `GTM_GROQ_API_KEY` or `GTM_GEMINI_API_KEY` | Groq / Google Gemini | Hosted LLM for the optional layer (`enable_llm: true`); Ollama locally needs no key | free, rate-limited | optional | ⏳ supported, not set |
